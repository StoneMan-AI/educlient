...
WECHAT_NOTIFY_URL=https://www.qingsongkao.cn/api/vip/payment-callback
...
```env
VITE_API_BASE_URL=https://www.qingsongkao.cn/api

# 支付测试模式（可选，仅用于开发测试）
# 设置为 true 时，支付界面会显示"一键支付成功（测试）"按钮
VITE_ENABLE_PAYMENT_MOCK=true
```

**注意**：
- `VITE_ENABLE_PAYMENT_MOCK` 用于控制前端是否显示模拟支付按钮
- 如果不需要测试功能，可以设置为 `false` 或不设置
- 修改环境变量后需要重新构建前端：`npm run build`
...
server {
    listen 80;
    server_name www.qingsongkao.cn;
...
# 申请SSL证书
sudo certbot --nginx -d www.qingsongkao.cn
sudo certbot --nginx -d www.qingsongkao.cn
