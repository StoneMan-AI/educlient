...
GRANT ALL PRIVILEGES ON DATABASE educlient TO educlient_user;
\q
```
...
# 执行基础表结构
sudo -u postgres psql -d educlient -f database/schema.sql

# 执行扩展表结构
sudo -u postgres psql -d educlient -f database/schema_extension.sql
```
...
```env
# 数据库配置
DATABASE_URL=postgresql://educlient_user:your_secure_password@localhost:5432/educlient
...
# 文件上传目录（如果使用）
UPLOAD_DIR=/var/www/educlient/uploads
```
...
```bash
# 项目已包含PM2配置文件 backend/ecosystem.config.js
# 如需修改配置，编辑该文件即可
...
```bash
# 测试后端API
curl https://yourdomain.com/api/grades
...
# 设置上传目录权限
sudo chmod 755 /var/www/educlient/uploads
```
...
```bash
# 测试数据库连接
psql -h localhost -U educlient_user -d educlient
...
mkdir -p $BACKUP_DIR
pg_dump -U educlient_user educlient > $BACKUP_DIR/educlient_$DATE.sql
# 保留最近7天的备份
find $BACKUP_DIR -name "educlient_*.sql" -mtime +7 -delete
```
